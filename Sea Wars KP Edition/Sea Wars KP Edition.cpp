...
void x_ray(int mass[n][n]);//показ поля. Выводит двумерный массив, показывает где корабли
...
int inputShips(int matrix[n][n])// Функция для ввода кораблей от игрока
...
int inputShips(int matrix[n][n]) {
    int x, y, direction, ship_size;
    cout << "Введите количество клеток корабля (1-4): ";
    cin >> ship_size;

    // Проверка на допустимый размер корабля
    if (ship_size < 1 || ship_size > 4) {
        cout << "Неверный размер корабля!" << endl;
        return -1; // Возвращаем ошибку
    }

    cout << "Введите координаты (x y) и направление (0 - горизонтально, 1 - вертикально): ";
    cin >> x >> y >> direction;

    // Проверка границ поля
    if (direction == 0) { // Горизонтально
        if (y + ship_size > n) {
            cout << "Ошибка размещения: вы выходите за границы!" << endl;
            return -1;
        }
        for (int i = 0; i < ship_size; ++i) {
            if (matrix[x][y + i] == 0) {
                matrix[x][y + i] = 1; // Установка части корабля
            }
            else {
        ...
        for (int i = 0; i < ship_size; ++i) {
            if (matrix[x + i][y] == 0) {
                matrix[x + i][y] = 1; // Установка части корабля
            }
            else {
                cout << "Ошибка размещения корабля!" << endl;
                return -1;
